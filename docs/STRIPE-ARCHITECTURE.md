...
Current state: Stripe payment state is still authoritative for payment completion, and invoice documents are still generated by the existing internal `billing_documents` + PDF pipeline until KROS issuance endpoints are integrated.
